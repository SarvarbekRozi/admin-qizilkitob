...
@section('title', 'Statistikani tahrirlash')
@section('page-title', 'Statistikani tahrirlash')
...
<div class="page-header">
    <div class="page-header-left">
        <h1>{{ $siteStat->label_uz }}</h1>
        <p class="subtitle">Statistikani tahrirlash</p>
    </div>
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <input type="text" name="key" class="form-control"
                               value="{{ old('key', $siteStat->key) }}" required>
                        <div class="form-hint">Masalan: animals, plants, zones</div>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Qiymat (value) <span class="req-badge">Majburiy</span></label>
                        <input type="text" name="value" id="val_input" class="form-control"
                               value="{{ old('value', $siteStat->value) }}" required
                               oninput="document.getElementById('preview_value').textContent = this.value || '0'">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Qo'shimcha belgi (suffix)</label>
                        <input type="text" name="suffix" id="suf_input" class="form-control"
                               value="{{ old('suffix', $siteStat->suffix) }}"
                               oninput="document.getElementById('preview_suffix').textContent = this.value">
                ...
                <div style="margin-top:16px; padding:16px; background:var(--body-bg); border-radius:10px; text-align:center;">
                    ...
                    <div style="font-size:32px; font-weight:700; color:var(--primary);">
                        <span id="preview_value">{{ old('value', $siteStat->value) }}</span><span id="preview_suffix" style="color:var(--secondary);">{{ old('suffix', $siteStat->suffix) }}</span>
                    </div>
                    <div style="font-size:13px; color:var(--text-muted);" id="preview_label">{{ old('label_uz', $siteStat->label_uz) }}</div>
                </div>
            </div>
        </div>
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <input type="text" name="label_uz" class="form-control"
                               value="{{ old('label_uz', $siteStat->label_uz) }}" required
            ...
            <div class="card-body">
                    ...
                    <div class="col-md-8">
                        ...
                        <div style="display:flex; gap:8px; align-items:center;">
                            ...
                            <div style="width:44px; height:44px; border:2px solid var(--border); border-radius:8px;
                                ...
                                <i id="icon_preview" class="{{ old('icon', $siteStat->icon) }}"
                ...
                <div style="display:flex; flex-wrap:wrap; gap:8px;">
                    @php
                        ...
                        $currentIcon = old('icon', $siteStat->icon);
                    @endphp
    ...
    <div class="col-lg-4">
        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                ...
                <input type="number" name="order" class="form-control"
                       value="{{ old('order', $siteStat->order) }}" min="0">
            ...
            <div class="card-body">
                <div class="form-check" style="display:flex; align-items:center; gap:10px; padding:12px;
                    ...
                    <input class="form-check-input" type="checkbox" name="is_active"
                           ...
                           {{ old('is_active', $siteStat->is_active) ? 'checked' : '' }}>
        ...
        <div class="card">
            ...
            <div class="card-body" style="font-size:12px; color:var(--text-muted);">
                <div>Yaratilgan: {{ $siteStat->created_at->format('d.m.Y H:i') }}</div>
                <div>Yangilangan: {{ $siteStat->updated_at->format('d.m.Y H:i') }}</div>
            </div>
        </div>
